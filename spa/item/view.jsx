                    ...
                    <section className="itemPageInfo">
                        ...
                        <article className="itemInfo">
                            <section className="itemFundamentals">
                                ...
                                <section className="itemFundamentalsThing">
                                    {item.dynamicData && item.dynamicData.isEditable && item.collection.extensionAddress && item.collection.extensionAddress !== window.voidEthereumAddress && <span>Host {item.collection.extensionIsContract ? "Contract" : "Wallet"}: <a target="_blank" href={`${window.getNetworkElement('etherscanURL')}address/${item.collection.extensionAddress}`}>{window.shortenWord(item.collection.extensionAddress, 10)}</a></span>}
                                    {((item.sourceAddress !== 'blank' && item.sourceAddress !== window.voidEthereumAddress) || item.collection.sourceAddress !== 'blank') && <span>Wrapped from: <a target="_blank" href={`${window.getNetworkElement('etherscanURL')}address/${item.sourceAddress !== 'blank' ? item.sourceAddress : item.collection.sourceAddress}`}>{item.sourceAddress !== 'blank' ? item.sourceAddress : window.shortenWord(item.collection.sourceAddress, 10)}</a></span>}
